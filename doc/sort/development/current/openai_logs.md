...
**Status:** Phase 1 shipped — header + per-message request counter  
...
**Restart the API** (ENV changed), then send one chat message that mentions a region (e.g. “what’s in us-west-2?”). Confirm Request 1 = Region Search, Request 2 = General Chat. Then commit Phase 1 when happy.
