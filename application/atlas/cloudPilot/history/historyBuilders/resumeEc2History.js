/*
FUNCTIONS A: Resume EC2 history builder — target, snapshots, undo payload
    1) Function A1: buildResumeEc2HistoryFields
*/

const TARGET_TYPE = 'ec2_instance';

//Function A1: Build history fields for a resume_ec2 execution
function buildResumeEc2HistoryFields(options) {
    ...
    const resourceStateAfter = buildStateSnapshot({
        instanceId: instanceId,
        region: region,
        state: stateAfter || (isCompleted ? 'running' : null),
        stateBefore: stateBefore,
        stateAfter: stateAfter || (isCompleted ? 'running' : null),
        noop: isNoop,
        changedByCloudPilot: changedByCloudPilot
    });
