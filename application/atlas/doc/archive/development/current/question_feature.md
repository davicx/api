# CloudPilot — Question Feature (Design)

**Status:** Design locked — implement Phase 1 when approved  
**Goal:** Name Conversation / Question / Action; keep understanding separate from product behavior; fix open-requests fallthrough to OpenAI  
**Work type:** Architecture design + phased build (Internal first)  
**Last updated:** 2026-08-03  
**Filename:** `question_feature.md`  
**Related:** [cloud_pilot_open_requests.md](./cloud_pilot_open_requests.md) · [cloud_pilot_capabilities.md](./cloud_pilot_capabilities.md) · [ai_usage.md](./ai_usage.md) · [current.md](./current.md) · [../architecture/responsibility_refactor.md](../architecture/responsibility_refactor.md)

This is the **one design doc** for this direction. Older naming draft: [conversation_query_action.md](./conversation_query_action.md) (redirects here).
...
│   ├── openRequests/        # NEW Phase 1 — delegates to requests/
...
Phase 1 does **not** require renaming every understand field to `kind` yet. Keep `conversation: 'list_open'` if needed; treat it as a Question signal in the router. Explicit `kind` can come later.
...
- [x] OpenAI off until Phase 1 green

### Phase 1 — Open Requests Question (fixes the incident)
...
### Phase 3 — More Questions (optional)
...
### Phase 4 — Explicit understand shape (optional)
...
Only if Phase 1–2 feel cramped. Not mixed into Phase 1.
...
- Turn general chat OpenAI back on before Phase 1 acceptance.
- Build factories/registries in Phase 1 — wire only.

---

## Acceptance (Phase 1 = incident fixed)
...
Say **go Phase 1** to implement the Open Requests Question (phrases + `questions/openRequests` + speak wire).  
OpenAI stays off.
